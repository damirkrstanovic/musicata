...
- `docs/` — research, requirements, roadmap, and per-feature designs. `testdata-fixture/` is
  the committed fixture library the UI smoke suite scans — 16 synthetic tagged tracks
  generated by `scripts/gen-testdata.sh`, so a fresh clone can run the suite. `testdata/` is
  git-ignored and holds whatever real library you point it at locally.
...
- Regenerate `THIRD-PARTY-NOTICES.md` with `node scripts/gen-third-party-notices.mjs` after
  changing `Cargo.lock` or the web `package-lock.json`. It carries the attribution the bundled
  dependencies' licenses require and ships in the release tarball and both images; `NOTICE`
  covers only third-party material checked into the repo. The `licenses` workflow regenerates
  it on every PR and tag and fails if it differs from the committed file, so a stale one can't
  ship — and `cargo deny check licenses` fails any dependency whose license isn't on the
  AGPL-compatible allow list in `deny.toml`.
...
- **A boolean setting is a `bool` in the database, and its default is declared once.** On/off
  toggles live in `bool_settings` (`INTEGER CHECK (value IN (0,1))`) — not in the string
  `settings` table — read via `bool_setting(db, KEY)` and written via `set_bool_setting`. Every
  default is a row in `BOOL_SETTINGS` in `main.rs`, with a one-line rationale; adding a toggle
  means adding a row there. The friction is the point: these previously lived as strings that
  each caller parsed itself, which produced five parsers, three notions of "true" and two
  answers for an absent row, so `""` meant *enabled* to one reader and *disabled* to another.
  Choosing a default is a product decision — especially for anything that talks to the
  network — and it belongs where a reviewer sees it, not in which helper you happened to call.
...
- **Capabilities are advertised** (`ProviderCapabilities`) and callers skip what a source
  can't do.
- **The web app never talks to the internet; the server does.** Every response carries
  `default-src 'self'`, so a `fetch()`, `<img>`, or `<audio>` pointed at a third-party origin
  *will* be blocked. Anything the page needs from outside is relayed by `src/proxy.rs` — radio
  (`/api/radio/{id}/stream`), podcast/Archive items (`/api/sources/{id}/stream`), AutoEq
  (`/api/autoeq/*`). The fix for a blocked load is a new relay endpoint, **not** a wider policy:
  the point is that an XSS past Svelte's escaping has nowhere to send what it reads. Two rules
  for a relay: the client names *what* it wants and never *where* to fetch it (else the endpoint
  is an open proxy inside the user's LAN — see `autoeq::preset_url`), and it streams without
  storing. `scripts/ui-smoke.sh` fails the build on any CSP violation.
...
Prefer unit tests beside the Rust code they cover. Scanner tests build their own libraries in
temp dirs, so `cargo test` needs no fixture on disk; the UI smoke suite scans the committed
`testdata-fixture/`, whose counts (9 artists / 6 albums / 16 tracks) the flows depend on — if
you change `scripts/gen-testdata.sh`, re-run the suite. Do not require private
