...
#   behavior — playback + flows against the committed `testdata-fixture/` library
#              (generated by scripts/gen-testdata.sh, so a fresh clone can run this).
...
fail=0
run_phase behavior 3979 --library testdata-fixture --database "$TMP/behavior.db" || fail=1
