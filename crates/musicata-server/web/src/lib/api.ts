...
export type { SourceView, AppSettings, ArtistAliasGroup, Activity, Player, Zone, About };

export interface CreateSourceRequest {
  kind: "smb" | "opensubsonic" | "podcast" | "archive";
  host?: string;
  share?: string;
  base_path?: string;
  display_name?: string;
...
export const api = {
  ...
  // Who this instance is + where its source lives (the AGPL section 13 offer). Open — this
  // one resolves before a session exists, so the login screen can show it too.
