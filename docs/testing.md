...
- [x] A fixture library for the `behavior` phase, so a fresh clone can run it. `testdata/`
  is git-ignored (it holds real, copyrighted music); `testdata-fixture/` is 16 synthetic
  tagged tracks generated by `scripts/gen-testdata.sh` and committed, which the suite now
  scans instead.
- [ ] A synthetic *large*-library fixture so the `scale` phase doesn't depend on a
  developer's personal `.musicata/musicata.db`. Still skips for want of a DB on a fresh
  clone — the remaining gap, though no longer a blocking one.
