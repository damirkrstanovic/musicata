...
On first run the server scans the configured library and stores canonical tracks plus observed metadata in SQLite. Later runs load from the database unless `--rescan` or `MUSICATA_RESCAN=true` is set.
